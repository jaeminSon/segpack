...
class DeepLab(nn.Module):
    ...
    def __init__(self, backbone='resnet', output_stride=16, num_classes=21,
        ...
        print("# trainable params: {}".format(np.sum([p.numel() for p in self.backbone.parameters() if p.requires_grad])))
        self.aspp = build_aspp(backbone, output_stride, BatchNorm)
        print("# trainable params: {}".format(np.sum([p.numel() for p in self.aspp.parameters() if p.requires_grad])))
        self.decoder = build_decoder(num_classes, backbone, BatchNorm)
        print("# trainable params: {}".format(np.sum([p.numel() for p in self.decoder.parameters() if p.requires_grad])))
